...
class TreasuresCommand(
    ...
    private fun handleSetBlock(sender: CommandSender, args: Array<out String>) {
        ...
        if (args.size != 2) {
            UniversalMessenger.send(
                    sender,
                    MessagesFile.getString("messages.commands_set_block_usage")
            )
            return
        }
        ...
        if (block == null) {
            UniversalMessenger.send(
                    sender,
                    MessagesFile.getString("messages.commands_set_block_block_is_null")
            )
            return
        }
                ...
                try {
                    ...
                    UniversalMessenger.send(
                            ...
                            MessagesFile.getString(
                                    "messages.commands_wrong_value",
                                    ReplaceData("{value}", args[1])
                            )
                    )
                    return
                }
        ...
        if (treasure == null) {
            UniversalMessenger.send(
                    sender,
                    MessagesFile.getString("messages.commands_treasure_not_found")
            )
            return
        }
    ...
    private fun handleGive(sender: CommandSender, args: Array<out String>) {
        ...
        if (target == null) {
            UniversalMessenger.send(
                    sender,
                    MessagesFile.getString("messages.commands_give_player_is_null")
            )
            return
        }
        ...
        return when (args.size) {
            ...
            2 ->
                    when (args[0].lowercase()) {
                        ...
                        "set-block" ->
                                TreasuresPlugin.instance.treasureRepository.getTreasures().map {
                                    it.uuid.toString()
                                }
                        else -> emptyList()
                    }
